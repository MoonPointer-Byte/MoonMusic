                ...
                for link in iusc_links:
                    try:
                        ...
                        if m_str:
                            ...
                            if img_url: results.append({"url": full_url, "thumb": img_url})
                    except:
                        continue
                if not results:
                    ...
                    for img in imgs:
                        ...
                        if src and src.startswith('http'): results.append({"url": src, "thumb": src})
    ...
    async def search_social_users(self, keyword, platform="all"):
        ...
        async def fetch_weibo(client):
            try:
                ...
                headers = {"User-Agent": "Mozilla/5.0 (Linux; Android 10; K)", "Referer": "https://m.weibo.cn/"}
                        ...
                        for item in card['card_group']:
                            if item.get('card_type') == 11 and 'user' in item:
                                ...
                                local_res.append({
                                    ...
                                    "desc": f"粉丝: {u.get('followers_count', 0)}",
        ...
        async with httpx.AsyncClient(verify=False, timeout=4.0) as client:
            tasks = []
            if platform in ["all", "bilibili"]: tasks.append(fetch_bili(client))
            if platform in ["all", "weibo"]: tasks.append(fetch_weibo(client))
            if tasks:
                task_results = await asyncio.gather(*tasks, return_exceptions=True)
                for tr in task_results:
                    if isinstance(tr, list): results.extend(tr)

            if platform in ["all", "douyin"]:
                results.append({"platform": "抖音", "name": f"搜索: {keyword}", "desc": "跳转抖音搜索", "pic": "",
                                "url": f"https://www.douyin.com/search/{urllib.parse.quote(keyword)}"})
            if platform in ["all", "xiaohongshu"]:
                results.append({"platform": "小红书", "name": f"搜索: {keyword}", "desc": "跳转小红书搜索", "pic": "",
